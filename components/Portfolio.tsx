...
const Portfolio: React.FC<PortfolioProps> = ({ lang, onProjectSelect, isFullView = true, onViewAll }) => {
  ...
  // Robust splitting logic for the section title
  const renderTitle = () => {
    const words = t.title.split(' ');
    if (lang === 'ar') {
      // Specifically balance "معالم معمارية"
      return (
        <>
          {words[0]} <br />
  ...
  return (
    <section id="projects" className="pt-24 pb-32 bg-zinc-950 relative overflow-hidden border-b border-white/5">
      ...
      <div className="container mx-auto px-6 md:px-10 relative z-10 max-w-[1600px]">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 space-y-12 md:space-y-0">
          <div className="animate-fade-in-up text-left rtl:text-right w-full md:w-auto">
            ...
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-architectural font-bold text-white tracking-tighter uppercase leading-[0.9] md:leading-tight">
              {renderTitle()}
            </h2>
          </div>
          
          <div className="flex flex-wrap gap-6 md:gap-10 border-b border-white/10 pb-4 w-full md:w-auto justify-start md:justify-end">
            {categories.map(cat => (
              <button
                key={cat.id}
                onClick={() => setActiveFilter(cat.id)}
                className={`text-[9px] font-architectural font-bold uppercase tracking-[0.25em] transition-all relative pb-4 ${
                  activeFilter === cat.id 
                    ? 'text-amber-500' 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 transition-all duration-500">
          {filteredProjects.map((project) => (
            ...
            >
              <div className="relative h-[350px] md:h-[500px] lg:h-[600px] overflow-hidden bg-zinc-900">
                <img 
                ...
                {project.has3D && (
                  <div className="absolute top-8 right-8 rtl:right-auto rtl:left-8 z-10 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                    <button 
                      ...
                      className="bg-amber-600 text-white px-6 py-3 rounded-sm text-[9px] font-architectural font-bold uppercase tracking-[0.2em] hover:bg-amber-700 transition-all flex items-center space-x-3 rtl:space-x-reverse shadow-[0_15px_35px_rgba(217,119,6,0.4)]"
                    >
              ...
              <div className="flex flex-col flex-1">
                <div className="p-8 md:p-12 border-b border-white/5 flex justify-between items-center group/title transition-all">
                  <div className="space-y-3 text-left rtl:text-right">
                    ...
                    <h3 className="text-3xl lg:text-4xl font-architectural font-bold text-white tracking-tighter uppercase leading-none pb-2 group-hover:text-amber-500 transition-colors duration-500">
                      {project.title}
                    </h3>
                  </div>
                  <div className="w-12 h-12 flex items-center justify-center bg-zinc-800/50 border border-zinc-700/50 text-white group-hover:bg-amber-600 group-hover:border-amber-500 transition-all duration-500 shrink-0 ml-6 rtl:ml-0 rtl:mr-6">
                    <svg className="w-5 h-5 rtl:rotate-180 transform group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                  </div>
                </div>

                <div className="px-8 md:px-12 py-8 bg-white/[0.01] flex flex-wrap gap-x-12 gap-y-6 justify-between items-center">
                  <div className="flex items-center space-x-12 rtl:space-x-reverse">
                    <div className="flex flex-col space-y-2 text-left rtl:text-right">
                      <span className="text-[9px] text-zinc-600 font-bold uppercase tracking-[0.2em] font-architectural">{t.ref}</span>
                      <span className="text-[12px] text-zinc-400 font-mono tracking-widest font-bold">ST-{project.year}-00{project.id}</span>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4 rtl:space-x-reverse border border-emerald-500/20 bg-emerald-500/5 px-4 py-2 rounded-sm backdrop-blur-md">
        ...
        {!isFullView && (
          <div className="mt-32 text-center animate-fade-in-up">
            <button 
              onClick={onViewAll}
              className="group relative inline-flex items-center bg-white text-zinc-950 px-16 py-7 text-[10px] font-architectural font-bold uppercase tracking-[0.4em] hover:bg-amber-600 hover:text-white transition-all overflow-hidden shadow-2xl"
            >
