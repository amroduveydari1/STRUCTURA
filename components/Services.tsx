...
const Services: React.FC<ServicesProps> = ({ lang }) => {
  const t = UI_STRINGS[lang].services;
      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16">
          <div className="max-w-2xl">
            <span className="text-amber-600 font-bold uppercase tracking-widest text-xs mb-4 block">{t.subtitle}</span>
            <h2 className="text-4xl md:text-5xl font-architectural font-bold text-zinc-900 tracking-tighter uppercase">
              {t.title}
            </h2>
          </div>
          <div className="hidden md:block">
            <p className="text-zinc-500 max-w-sm text-right rtl:text-left font-light leading-relaxed">
              {t.description}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-200 border border-zinc-200">
          {SERVICES(lang).map((service) => (
            ...
            >
              <div className="text-amber-600 mb-8 group-hover:scale-110 transition-transform duration-500 origin-left rtl:origin-right">
                {service.icon}
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-zinc-200 group-hover:border-zinc-800 transition-colors duration-500">
                <button 
                  ...
                  className="text-sm font-bold uppercase tracking-widest text-amber-600 group-hover:text-amber-500 flex items-center space-x-2 rtl:space-x-reverse w-full text-left rtl:text-right"
                >
                  <span>{t.download}</span>
                  <svg className="w-4 h-4 transform group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4-4m4-4H3"/></svg>
                </button>
              </div>
            </div>
          ))}
